...
- [x] Added `--short` flag to `pysmelly init` that generates a ~15-line summary instead of the full guidance file. Key points only: severity tiers, "default is to fix", incremental workflow. Links to pysmelly's README for details.
